...
app.use(cors({
  origin: process.env.FRONTEND_URL || true, // Use FRONTEND_URL from env, fallback to allow all
  credentials: true
}));
...
app.get('/', (req, res) => {
  res.json({ message: 'ClinicFlow Backend API is running on Firebase Functions!' });
});
