...
app.post('/api/auth/reset-password', async (req, res) => {
    try {
        ...
        const { error: updateError } = await supabase.auth.admin.updateUserById(tokenData.email, { password: newPassword });
        if (updateError) {
            ...
            const { error: userUpdateError } = await supabase.auth.admin.updateUserById(user.id, { password: newPassword });
            if (userUpdateError) {
                return res.status(500).json({ error: 'Failed to update password' });
            }
        }
